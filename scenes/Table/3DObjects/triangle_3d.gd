...
@onready var _mat_2d = preload("res://materials/table_2d_material.tres").duplicate()
...
func draw_2d():
	...
	# Shader
	mesh_2d.material_override.set_shader_parameter("vertex_0", global_vertices[0])
	mesh_2d.material_override.set_shader_parameter("vertex_1", global_vertices[1])
	mesh_2d.material_override.set_shader_parameter("vertex_2", global_vertices[2])

	mesh_2d.material_override.set_shader_parameter("normal", FlatReference.plane.normal)
	mesh_2d.material_override.set_shader_parameter("d", FlatReference.plane.d)

	mesh_2d.material_override.set_shader_parameter("y_frustrum", FlatReference.y_frustrum)
	...
	for i in range(3):
		...
		_mesh_2d.surface_add_vertex(global_transform.affine_inverse() * project(global_vertices[i]))
